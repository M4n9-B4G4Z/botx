...
──────────────────────
Hi👋, ${pushname}
Kamu belum terdaftar sebagai Teman Nafiz
untuk mendaftar kirim 

-${prefix}daftar |nama|umur

contoh format: 
-${prefix}daftar |nafiz|17
...
──────────────────────`, id) //if user is not registered
        ...
        function cekumur(obj){
            if (obj === true){
                ...
                return tobz.reply(self, `
*Maaf ${pushname}!
*Kamu belum cukup umur untuk menggunakan Bot Nafiz.
Minimal Umur 16 tahun
Kamu bisa mendaftar ulang dengan cara donasi terlebih dahulu
bales 
├❏ ${prefix}donate

Hubungi Owner : wa.me/6281220439155`, id) //if user is not registered
            }
        }    
        ...
        const ownerNumber = '6281220439155@c.us'
        ...
        // FUNCTION
                ...
                for (let i of stickerspam){
                    if(i.id === id){
                        if (i.msg >= 5) {
                            found === true 
                            tobz.reply(self, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh Nafiz`, id).then(() => {
                                tobz.removeParticipant(groupId, id)
                            })
                            return true;
                        }else if(i.msg >= 5){
                            found === true
                            tobz.reply(self, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu terdeteksi spam sticker!\nMohon tidak spam 5 sticker lagi atau nomor akan di kick oleh Nafiz!`, id)
                            return true
                        }else{
                            found === true
                            return false;
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 1) { // 1X BADWORD AKAN TERKENA KICK
                        kasar === true 
                        tobz.reply(self, `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nPeringatan! 2x lagi berkata kasar, Nafiz akan kick kamu dari grup!`, id)
                        }
                    if (msg >= 3) { // 3X BADWORD AKAN TERKENA KICK
                        kasar === true 
                        tobz.reply(self, `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh Nafiz!`, id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        })
                        return true;
                    }else{
       ...
       if (isGroupMsg && GroupLinkDetector && !isGroupAdmins && !isAdmin && !isOwner){
        ...
        if(!isCmd && isGroupMsg && isBadword && !isGroupAdmins) { 
            ...
            if(isBadwordMsg(serial)) return
                ...
                if(body === `${prefix}mute` && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(self, 'Maaf, perintah ini hanya dapat dilakukan oleh admin Nafiz!', id)
				...
				/*if (chats.match("Nafiz") || chats.match("Nafiz") || chats.match("Nafiz") || chats.match("bot") || chats.match("Bot") || chats.match("BOT")) {
					const bls = balas[Math.floor(Math.random() * (balas.length))]
					await tobz.reply(self, `*${bls}* \n\n`, id)
				}*/

                if (body === `${prefix}public`) {
                    if (!isOwner) return tobz.reply(self, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner Nafiz!', id)
        ...
        case `${prefix}self`:
            ...
            if (!isOwner) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan oleh Owner Nafiz!', id)
            ...
            break
		case `prefix`:
			tobz.reply(self, `*Nafiz is Use ( ${prefix} ) Prefix!.* 
_Prefix adalah tanda di awal perintah._
_Contoh: ${prefix}menu_`, id)
			break
		case `${prefix}setto`:
        case `${prefix}setp`:
			if (!isOwner && !isAdmin) return tobz.reply(self, 'Maaf, Fitur ini hanya untuk OWNER dan ADMIN Nafiz!', id)
...
Contoh: ${prefix}setto #`, id)
            ...
            break
                ...
                case `${prefix}nobg`:
                    ...
                    if (isMedia) {
                        try {
                            nobg.Removebg()
                            tobz.sendFileFromUrl(dari, image, id)
                        } catch (err) {
                            console.log(err)
                        }
                    }
                    break           
        case `${prefix}esticker`: //NAFIZ
            //if(isReg(obj)) return
            //if(cekumur(cekage)) return
            //if (!isRegis) return tobz.reply(self, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Nafiz, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            tobz.sendStickerfromUrl(self, `https://api.vhtear.com/emojitopng?code=${emoji}&apikey=${vhtear}`)
            break            
        /*case `${prefix}ttp`:
                tobz.reply(self, '*Sedang Eror!*', id)
                if (isLimit(serial)) return tobz.reply(self, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis/nJika Ingin Isi Ulang Chat Owner!/nKetik #limit Untuk Mengecek Kuota Limit Kamu`, id)
        ...
        case `${prefix}ttp`:
            ...
            ////if (!isVipUser) return tobz.reply(self, '*Fitur Ini Khusus VIP!*\n _Daftar VIP Hanya 5k/Bulan_\n\n*Mau Daftar VIP?*\nChat Owner BOT\n_wa.me/6281220439155_ ', id)
            if (isLimit(serial)) return tobz.reply(self, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis/nJika Ingin Isi Ulang Chat Owner!/nKetik #limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(self, `Kirim perintah *${prefix}ttp2 [ Teks ]*, contoh *.ttp2 Nafiz*`, id)
        ...
        case `${prefix}ttg`:
            ...
            try {
                if (quotedMsgObj == null) {
                    if (args.length === 1) return tobz.reply(self, `Kirim perintah *${prefix}ttg [ Teks ]* , contoh *${prefix}ttg aku bukan boneka*`, id)
...
──────────────────`)
        ...
        case `${prefix}creator`:
            //if(isReg(obj)) return
            //if(cekumur(cekage)) return
            tobz.sendContact(chatId, `6281220439155@c.us`)
        ...
        case `${prefix}nsfw`:
            ...
            if (args[1].toLowerCase() === 'enable') {
                ...
                tobz.reply(self, 'NSFW berhasil di aktifkan di group ini! kirim perintah *@nsfwMenu* untuk mengetahui menu', id)
        ...
        case `${prefix}simi`:
            ...
            //if(cekumur(cekage)) return
            //if (!isGroupMsg) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan dalam group!', id)
           // if (!isAdmin) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan oleh Admin Nafiz!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(self, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
        ...
        case `${prefix}rbokep`:    
            ...
            if (isLimit(serial)) return tobz.reply(self, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis/nJika Ingin Isi Ulang Chat Owner!/nKetik #limit Untuk Mengecek Kuota Limit Kamu`, id) // MFARELS
...
🌐️ *URL*: ${url}`
            ...
            try {
                ...
                const webplay2 = await webplay.json()
                 if (webplay2.status == false) {
                    ...
                    await tobz.sendFileFromUrl(self, mp3, `${title}.mp3`, '', id).catch(() => tobz.reply(self, mess.error.Yt4, id))
                }
            } catch (err) {
        ...
        /*case `${prefix}nyanyi`:
            ...
            try {
                tobz.reply(self, mess.wait, id)
                const datanyanyi = await get.get(`https://api.vhtear.com/music?query=${quernyanyi}&apikey={VhTear}`)
        ...
        case prefix+'getnhentai': // DOWNLOADER NHENTAI PDF FROM #NHENTAI
            ...
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/6281311850715', id)
          break
        case `${prefix}phdl`: //thanks to tobz BOT
            if (!isRegis) return tobz.reply(self, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Nafiz, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
        ...
        case prefix+'getxvideos': // DOWNLOADER VIDEO FROM #VIDEO
            ...
            case `${prefix}film`:{
                if(isLimit(serial)) return
                if (args.lenght === 0) return tobz.reply(self, 'Gunakan Fromat #film [nama film]\nContoh #film Downhill', id)
        ...
        /*case `${prefix}animesearch`: // SEARCH ANIME
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(self, 'Kirim perintah *#animesearch [query]*, Contoh : #animesearch DXD (Hanya Bisa Satu Kata)')
        ...
        case `${prefix}tiktok`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(self, 'Kirim perintah *#tiktok [linkTiktok]*\nContoh : *#tiktok https://vt.tiktok.com/yqyjPX/*', id)
            tobz.reply(self, mess.wait, id)
        ...
        case `${prefix}smule`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(self, 'Kirim perintah *#smule [linkSmule]*\nContoh : *#smule https://www.smule.com/p/767512225_3062360163*', id)
            tobz.reply(self, mess.wait, id)
        ...
        case `${prefix}sandwriting`:
            ...
            await limitAdd(serial)
            if (args.length === 1)  return tobz.reply(self, 'Kirim perintah *#sandwriting [ Teks ]*\nContoh *#sandwriting Nafiz Cantik*', id)
...
➸ *Jumlah Postingan:* ${status_count}`
            ...
            const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
            tobz.sendImage(self, base64, name, twstalk3)
            } catch (err) {
             console.error(err.message)
             await tobz.sendFileFromUrl(self, errorurl2, 'error.png', '💔️ Maaf, User tidak ditemukan')
...
➸ *Total Rekaman:* ${recording}`
        ...
        //if (!isVipUser) return tobz.reply(self, '*Fitur Ini Khusus VIP!*\n _Daftar VIP Hanya 5k/Bulan_\n\n*Mau Daftar VIP?*\nChat Owner BOT\n_wa.me/6281220439155_ ', id)
           ...
           // if (!isSimi) return tobz.reply(self, 'command/Perintah Simi belum di aktifkan di group ini!', id)
            if (args.length === 1) return tobz.reply(self, 'Kirim perintah *# [teks]*\nContoh : *# halo*')
        ...
        case `${prefix}instagram`:
            if (isLimit(serial)) return tobz.reply(self, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis/nJika Ingin Isi Ulang Chat Owner!/nKetik #limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(self, `Kirim perintah *#ig [ Link Instagram ]* untuk contoh silahkan kirim perintah *.readme*`)
            if (!args[1].match(isUrl) && !args[1].includes('instagram.com')) return tobz.reply(self, `Maaf, link yang kamu kirim tidak valid. [Invalid Link]`, id)
            //await tobz.reply(self, mess.wait, id);
        ...
        case `${prefix}carifoto`: {
            if(isLimit(serial)) return
                        ...
                        if (args.length == 0) return await tobz.reply(self, 'Fromat: #carifoto [teks]\ncontoh\n#carifoto mina twice')
        ...
        case `${prefix}ig2`: 
            if(isLimit(serial)) return
            if (args.length === 0) return tobz.reply(self, 'Kirim perintah *#ig [link ig]*')
        ...
        case `${prefix}twitter`:
            //if (!isGroupMsg) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(self, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis/nJika Ingin Isi Ulang Chat Owner!/nKetik #limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(self, `Kirim perintah *#twitter [ Link Twitter ]* untuk contoh silahkan kirim perintah *.readme*`)
            tobz.reply(self, mess.wait, id)
...
➸ *Durasi:* ${duration}`
            ...
            tobz.sendFileFromUrl(self, linkMp3, `${judul}.mp3`, '', id)
            } catch (err) {
             console.error(err.message)
             await tobz.sendFileFromUrl(self, errorurl2, 'error.png', '💔️ Maaf, User tidak ditemukan')
...
➸ *Wilayah:* +${region_name}`
        ...
        case `${prefix}nulis`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(self, 'Kirim perintah *#nulis [teks]*, contoh *#nulis aku bukan boneka*', id)
            ...
            if (ngetikk.data.error) return tobz.reply(self, ngetikk.data.error, id)
            tobz.sendFileFromUrl(self, ngetikk.data.result, 'nulis.jpg', '', id)
            break
                case '#inu':
            if (!isGroupMsg) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(self, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis/nJika Ingin Isi Ulang Chat Owner!/nKetik #limit Untuk Mengecek Kuota Limit Kamu`, id)
        ...
        case `${prefix}magernulis1`: // BY MFARELS
                if (args.length === 4) return await tobz.reply(self, 'Kirim Perintah *#magernulis1 --[Nama]--[Kelas]--[Teks]*\n\n*Contoh :*\n#magernulis1 --MFarelS--7B--Subscribe MFarelS CH', id) // https://github.com/MFarelS/RajinNulis-BOT
        ...
        case `${prefix}zodiak`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(self, 'Kirim perintah *#zodiak [zodiak kamu]*\nContoh : *#zodiak scorpio*', id)
            try {
        ...
        case `${prefix}chord`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(self, 'Kirim perintah *#chord [query]*, contoh *#chord aku bukan boneka*', id)
        ...
        // ADMIN & OWNER
        case `${prefix}bc`:
            if (!isOwner) return tobz.reply(self, 'Perintah ini hanya untuk Owner Nafiz!', id)
            ...
            for (let ids of chatz) {
                ...
                if (!cvk.isReadOnly) 
                await tobz.sendFile(ids, `${imageBase64}`,'bc.jpg', `[ *Nafiz BroadCast* ]\n\n${msg}`)
            }
            } else {
            let msg = body.slice(4)
            const chatz = await tobz.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await tobz.getChatById(ids)
                if (!cvk.isReadOnly) 
                await tobz.sendText(ids, `[ *Nafiz BroadCast* ]\n\n${msg}`)
            }
        }
        ...
        case `${prefix}ownergroup`:
            ...
            break
        case `${prefix}otagall`: // FOR OWNER & ADMIN Nafiz
        case `${prefix}omentionall`:
		case `${prefix}p`:
            if (!isGroupMsg) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(self, 'Perintah ini hanya untuk Owner Nafiz', id)
        ...
        case `${prefix}okickall`:
            if (!isGroupMsg) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(self, 'Perintah ini hanya untuk Admin Nafiz', id)
            if (!isBotGroupAdmins) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case `${prefix}oadd`:
            ...
            if (!isOwner, !isAdmin) return tobz.reply(self, 'Perintah ini hanya untuk Admin Nafiz', id)
            if (!isBotGroupAdmins) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case `${prefix}oleave`:
            if (!isGroupMsg) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(self, 'Perintah ini hanya untuk Admin Nafiz', id)
            await tobz.sendText(self,'Nafiz DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case `${prefix}keluar`:
        case `${prefix}out`:
            if (!isGroupMsg) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isAdmin) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan oleh admin Nafiz', id)
        ...
        case `${prefix}getss`:
            ...
            break
        case `${prefix}nafizadmin`:
            let admn = `This is list of Nafiz Admin\nTotal : ${adminNumber.length}\n`
            ...
            break
        /*case `${prefix}limit`:
			if (!isAdmin) return tobz.reply(self, 'Jika ingin cek limit bilang ke Admin atau Owner Nafiz ya Kak...', id)
            var found = false
...
Ketik #ceklimit Untuk Mengecek Kuota Limit Kamu
────────────────────`, id)
                    tobz.reply(self, `
╭─────────────────── 
├─❍ *Name* : *${pushname}
...
╰───────────────────`, id)
		...
		case `${prefix}addlimit`:
            //if (!isVipUser) return tobz.reply(self, '*Fitur Ini Khusus VIP!*\n _Daftar VIP Hanya 5k/Bulan_\n\n*Mau Daftar VIP?*\nChat Owner BOT\n_wa.me/6281220439155_ ', id)
			if (!isAdmin) return tobz.reply(self, 'Maaf kak, hanya untuk Admin Nafiz.', id)
			var found = false;
                    Object.keys(limit).forEach((i) => {
...
Terima kasih. Gunakan dengan sebaik mungkin :)`
        ...
        case `${prefix}addsay`:
            ////if (!isRegis) return tobz.reply(self, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Nafiz, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (args.length == 1) return tobz.reply(self, `Kirim perintah *!addsay [teks]*, contoh *#addsay anjay*`, id)
		...
		/*case `${prefix}addvip`:
            ...
            if (!isOwner) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan oleh Owner Nafiz!', id)
				...
				tobz.reply(self, 'Success Menambahkan User VIP Nafiz!', id)
				}
            break*/           
...
₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋
Hi ${pushname}
Kamu terlalu tua, kembali lagi ke masa muda untuk menggunakan Bot Nafiz
`, id)
...
₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋`, id) //if user is not registered
             ...
             case `${prefix}daftarulang`:
                    if (!isOwner) return tobz.reply(self, 'Command ini hanya dapat digunakan oleh Owner Nafiz', id)  
                    const daftar = args[1]
...
──────────────────`, id)
            break   
        /*/case `${prefix}listvip`:
            let lv = `Ini adalah list User VIP Nafiz\nTotal : ${VipUser.length}\n`
		...
		case `${prefix}addadmin`:
            if (!isOwner) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan oleh Owner Nafiz!', id)
                ...
                tobz.reply(self, 'Success Menambahkan Admin Nafiz!', id)
				}
            break
        case `${prefix}deladmin`:
            if (!isOwner) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan oleh Owner Nafiz!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(self, 'Success Menghapus Admin Nafiz!', id)
            break
        case `${prefix}block`:
            if (!isOwner) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan oleh Owner Nafiz!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
...
`)}`, id)
	    ...
	    case `${prefix}you`:
			if (!isAdmin) return tobz.reply(self, 'Perintah ini hanya bisa di gunakan oleh AdminNafiz!', id)
...
*Langgar auto Kick* kecuali owner`, id)
            break            
        case `${prefix}nafizgroup`:
        case `${prefix}nafizgrup`:
            tobz.reply(self, `Link Group *𝐍𝐀𝐅𝐈𝐙-𝐁𝐎𝐓*: https://chat.whatsapp.com/CSYI4Ut6jtP3Cu5KG576kK\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break   
...
*Chat Owner BOT :*
_wa.me/6281220439155_

*GRUP WHATSAPP BOT :*
_https://chat.whatsapp.com/CSYI4Ut6jtP3Cu5KG576kK_`, id)
            break
...
${format(uptime)}`)}`, id)
		...
		case `${prefix}stylewriting`:
            ...
            await limitAdd(serial)
            if (args.length === 1)  return tobz.reply(self, 'Kirim perintah *#sandwriting [ Teks ]*\nContoh *#sandwriting NAFIZGANS*', id)
   ...
   *Text : ${swrt1}*`
        ...
        case `linkgrup dong`: 
            ...
            break  
        case `salken`:  
            tobz.reply(self, `*WA-BOT*\n_Hi Salken Kak, Aku Robot Whatsappnya Nafiz:v_`, id)
            break      
        case 'Nafiz':     
        case 'napiz':
        case 'nafiz':
...
// By Gimenz
        ...
        case `${prefix}sdesk`:
            ...
            if(!isGroupAdmins) {
				var wkk = `${from.split('-')[0]}@c.us`
				if(message.author == wkk || message.author == '6281220439155@c.us') {
					try {
    ...
    //VIP                             
        ...
        case `${prefix}tekshub`: 
            //if (!isVipUser) return tobz.reply(self, '*Fitur Ini Khusus VIP!*\n _Daftar VIP Hanya 5k/Bulan_\n\n*Mau Daftar VIP?*\nChat Owner BOT\n_wa.me/6281220439155_ ', id)       
            if (isLimit(serial)) return tobz.reply(self, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis/nJika Ingin Isi Ulang Chat Owner!/nKetik #limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(self, `Kirim perintah *.pornhub [ |Teks1|Teks2 ]*, contoh *.pornhub |Nafiz|Bot*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                tobz.reply(self, mess.wait, id)
                ...
                await tobz.reply(self, `Wrong Format!\n[❗] Kirim perintah *.pornhub [ |Teks1|Teks2 ]*, contoh *.pornhub |Nafiz|Bot*`, id)
            }
            break             
        ...
        case `${prefix}bpink`:
            ...
            if (args.length === 1) return tobz.reply(self, `kirim perintah ${prefix}blackpink [text]. Contoh: *${prefix}blackpink Nafiz*`, id)
...
Text : ${txt1}`
        ...
        case `${prefix}searchteks`: 
         case `${prefix}googleteks`:
            ////if (!isVipUser) return tobz.reply(self, '*Fitur Ini Khusus VIP!*\n _Daftar VIP Hanya 5k/Bulan_\n\n*Mau Daftar VIP?*\nChat Owner BOT\n_wa.me/6281220439155_ ', id)       
            if (isLimit(serial)) return tobz.reply(self, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis/nJika Ingin Isi Ulang Chat Owner!/nKetik #limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(self, `Kirim perintah *#searchteks [ |Teks1|Teks2|teks3 ]*, contoh *#searchteks |Nafiz|Gans|Banget*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 3) {
                tobz.reply(self, mess.wait, id)
         ...
         case `${prefix}harta`:
            ////if (!isVipUser) return tobz.reply(self, '*Fitur Ini Khusus VIP!*\n _Daftar VIP Hanya 5k/Bulan_\n\n*Mau Daftar VIP?*\nChat Owner BOT\n_wa.me/6281220439155_ ', id)
             ...
             if (!jreng) return tobz.reply(self, 'Kirim perintah *#tahta [teks]*\n\nContoh *#tahta Nafiz*', id)
         ...
         case `${prefix}silkteks`:
            ////if (!isVipUser) return tobz.reply(self, '*Fitur Ini Khusus VIP!*\n _Daftar VIP Hanya 5k/Bulan_\n\n*Mau Daftar VIP?*\nChat Owner BOT\n_wa.me/6281220439155_ ', id)
             ...
             if (!silktext) return tobz.reply(self, 'Kirim perintah *#silkteks [teks]*\n\nContoh *#silkteks Nafiz*', id)
             //tobz.sendText(self, '*Sedang di proses*...', id)
         ...
         case `${prefix}partyteks`:
            ...
            ////if (!isVipUser) return tobz.reply(self, '*Fitur Ini Khusus VIP!*\n _Daftar VIP Hanya 5k/Bulan_\n\n*Mau Daftar VIP?*\nChat Owner BOT\n_wa.me/6281220439155_ ', id)
             ...
             if (!partytext) return tobz.reply(self, 'Kirim perintah *#partyteks [teks]*\n\nContoh *#partyteks Nafiz*', id)
             //tobz.sendText(self, '*Sedang di proses*...', id)
         ...
         case `${prefix}romanceteks`:
            ////if (!isVipUser) return tobz.reply(self, '*Fitur Ini Khusus VIP!*\n _Daftar VIP Hanya 5k/Bulan_\n\n*Mau Daftar VIP?*\nChat Owner BOT\n_wa.me/6281220439155_ ', id)
             ...
             if (!romancetext) return tobz.reply(self, 'Kirim perintah *#romanceteks [teks]*\n\nContoh *#romanceteks Nafiz*', id)
             //tobz.sendText(self, '*Sedang di proses*...', id)
        ...
        case `${prefix}glitchteks`:
            ...
            if (argz.length >= 3) {
                ...
                await tobz.reply(self, `Wrong Format!\n[❗] Kirim perintah *#glitchteks  |Teks1|Teks2*, contoh *#glitchteks |Nafiz|Bot*`, id)
            }
            break   
        ...
        case `${prefix}valenmaker`:
            ...
            if (argz.length >= 3) {
                ...
                await tobz.reply(self, `Wrong Format!\n[❗] Kirim perintah *#valentinemaker  |Teks1|Teks2*, contoh *#valentinemaker |Nafiz|Bot*`, id)
            }
            break                                                                          
        case `${prefix}fssarah`:
                ...
                if (quotedMsg && quotedMsg.type == 'image') {
                    ...
                    const vidxvideo = await vidxvid.json()
                     if (vidxvideo.status == false) {
                        ...
                        // await tobz.sendFileFromUrl(self, result, `${title}.mp3`, `XVIDEOS BY TOBZ`, id).catch(() => tobz.reply(self, mess.error.Yt4, id))
                        await limitAdd(serial)
                        } catch (err){
                            console.log(err)
                        }
		...
		default:
            //if (!isGroupMsg) return tobz.reply(self, `Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Nafiz, Link Ada Di Bio atau Bisa Mengetik ${prefix}Nafizgroup!`, id)
